...
/**
 * AZChat download tracker (Cloudflare Worker).
 *
 * GET  /download increments KV, serves gzip via ASSETS.fetch (no 302)
 * /v1, /mcp, and /v1/mesh/* do not increment.
 * GET  /count   JSON {project, views, downloads, total} — total = downloads
 * GET  /stats   JSON totals + per-repo + per-branch breakdown
 * POST /event   forks report a download {owner,repo,branch,fork,asset}
 *
 * Isolated: Worker azchat-download-tracker, KV AZCHAT_DOWNLOADS
 * (id 01a3bbf8ad5e449eb0eb7648d75f8ae0). Not mixed with any other product.
 *
...
const PROJECT = "azchat";
const DEFAULT_ASSET = "azchat-0.1.0.tar.gz";
const DEFAULT_OWNER = "AzielEliab";
const DEFAULT_REPO = "azchat";
...
async function githubStats(env) {
  ...
  const headers = { "User-Agent": "Mozilla/5.0 AZChat-download-tracker", Accept: "application/vnd.github+json" };
  let stars = 0;
  let forks = 0;
  let watchers = 0;
  let release_download_count = 0;
  try {
    const repoRes = await fetch("https://api.github.com/repos/AzielEliab/azchat", { headers });
    if (repoRes.ok) {
    ...
    const relRes = await fetch("https://api.github.com/repos/AzielEliab/azchat/releases/latest", { headers });
    if (relRes.ok) {
...
async function collectStats(env) {
  ...
  return {
    ...
    note: "Isolated AZChat counter. Forks identified by GitHub owner/repo. Key layout: project|owner|repo|branch|fork. Views are separate from downloads. /v1 does not increment.",
  };
}

function installScript() {
  return `#!/usr/bin/env bash
# AZChat one-click install. Counted download via this Worker.
set -euo pipefail
HOST="${HOST}"
ASSET="${DEFAULT_ASSET}"
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/" && request.method === "GET") {
      ...
      return new Response(renderHome(await collectStats(env)), {
